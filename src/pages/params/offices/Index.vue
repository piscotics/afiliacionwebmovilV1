...
import ModalZones from "./ModalOffices";
...
export default {
  ...
  data() {
    return {
      loadingTable: false,
      pagination: [5, 10, 0],
      viewColumns: ["actions", "nombreSucursal"],
      columns: [
        {
          name: "actions",
          label: "",
  ...
  methods: {
    ...
    saveOrEditItem(valueItem) {
      this.$q
        .dialog({
          component: ModalZones,
          valueField: this.fabAction ? "" : valueItem.nombreSucursal,
          parent: this,
    ...
    async addRegister(dataValue) {
      await Offices.create(dataValue).then((response) => {
    ...
    async editRegister(dataValue, idValue) {
      await Offices.edit(idValue, dataValue).then((response) => {
        console.log(response)
        if (response?.status === 200) {
    ...
    async loadData() {
      ...
      await Offices.list().then((response) => {
        if (response?.status === 200) {
