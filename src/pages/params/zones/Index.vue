...
export default {
  ...
  data() {
    return {
      loadingTable: false,
      pagination: [5, 10, 0],
      viewColumns: ["actions", "nombreZona"],
      columns: [
        {
          name: "actions",
          label: "",
  ...
  methods: {
    ...
    saveOrEditItem(valueItem) {
      this.$q
        .dialog({
          ...
          valueField: this.fabAction ? "" : valueItem.nombreZona,
          parent: this,
    ...
    async addRegister(dataValue) {
      ...
      await Zones.create(dataValue).then((response) => {
    ...
    async editRegister(dataValue, idValue) {
      await Zones.edit(idValue, dataValue).then((response) => {
        console.log(response)
        if (response?.status === 200) {
    ...
    async loadData() {
      ...
      await Zones.list().then((response) => {
        if (response?.status === 200) {
